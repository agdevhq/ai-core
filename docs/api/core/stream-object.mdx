...
<ResponseField name="result" type="Promise<GenerateObjectResult<TSchema>>">
  Resolves with the final validated object result when the stream completes.
</ResponseField>

<ResponseField name="events" type="Promise<readonly ObjectStreamEvent<TSchema>[]>">
  Resolves with all observed events.
</ResponseField>
...
<Info>
  The `object` event is emitted once at the end with the complete validated object. Use `object-delta` events to show progressive updates.
</Info>

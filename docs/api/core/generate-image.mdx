...
The `generateImage()` function generates images from text prompts using image generation models. It supports generating multiple images, different sizes, and provides both base64 and URL outputs.
...
<Info>
  The `revisedPrompt` field is available when the model enhances short prompts with more detail.
</Info>
...
4. **Check Revised Prompts**: Learn from the model's prompt enhancements

...
The `providerMetadata` is provider-namespaced. The top-level key is the provider identifier (for example `'anthropic'`, `'google'`, or `'openai'`). Use [`getProviderMetadata()`](/api/core/utilities) for typed access.

...
The `providerMetadata` is provider-namespaced. The top-level key is the provider identifier (e.g. `'anthropic'`, `'openai'`). Use the `getProviderMetadata` helper for type-safe access:

...
<ResponseField name="reasoning-end" type="{ type: 'reasoning-end'; providerMetadata?: Record<string, Record<string, unknown>> }">
...
<Info>
  The stream starts the HTTP request immediately when created. You do not need to iterate to begin fetching data.
</Info>
...
<Info>
  `.result` resolves with the aggregated final response when the stream completes, regardless of whether you consumed the events via iteration. `.events` resolves with all observed events, even on abort or error.
</Info>
...
```typescript
import { LLMError, StreamAbortedError } from '@core-ai/core-ai';

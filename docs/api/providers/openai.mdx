...
<Note>
  The Chat Completions API sends `reasoning_effort` in the request but does not return reasoning content in the response. `reasoning` is always `null` in the result when using `createOpenAICompat`.
</Note>
...
<Warning>
  GPT-5 series models (`gpt-5.4`, `gpt-5.2`, `gpt-5.1`, `gpt-5`, `gpt-5-mini`, `gpt-5-nano`) restrict sampling parameters when reasoning is enabled. Setting `temperature` or `topP` throws a `ProviderError`.
</Warning>
...
<Note>
  The Responses API sets `store: false` by default. Set `store: true` explicitly if you want to store responses.
</Note>

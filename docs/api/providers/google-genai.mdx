...
Gemini 3.x uses HIGH/LOW thinking control. Mapping: `minimal`/`low`/`medium` -> LOW, `high`/`max` -> HIGH.

<Note>
  Gemini 3.x models cannot disable thinking completely.
</Note>
...
| Gemini 3.1 Flash Lite Preview | Level (HIGH/LOW) | No | Cost-efficient, fast |

...
<AccordionGroup>
  <Accordion title="Gemini 3 (Thinking Level)" icon="star">
    ...
    - **gemini-3.1-flash-lite-preview** - Cost-efficient, fast with thinking level control
    - **gemini-3-pro** - Previous generation with thinking level control
  </Accordion>
...
Gemini 3 uses HIGH/LOW thinking control. Mapping: `minimal`/`low`/`medium` -> LOW, `high`/`max` -> HIGH.
...
for (const part of result.parts) {
  if (part.type === 'reasoning') {
    const meta = getProviderMetadata<GoogleReasoningMetadata>(
      part.providerMetadata,
      'google'
    );
    console.log(meta?.thoughtSignature);
  }
}
```
...
| Gemini 3.1 Flash Lite | Level (HIGH/LOW) | No | Cost-efficient tasks |
| Gemini 3 Pro | Level (HIGH/LOW) | No | Complex multimodal |

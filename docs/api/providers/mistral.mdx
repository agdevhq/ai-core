...
| Vision | Limited |
| Reasoning | Extraction only |
| Embeddings | Yes |
| Image Generation | No |

<Note>
  Mistral models extract reasoning from thinking content chunks but do not support explicit reasoning effort control. The `reasoning` config is accepted but effort levels are not mapped to the Mistral API.
</Note>

...
| Vision | Limited |
| Reasoning | Yes (native thinking) |
| Embeddings | Yes |
| Image Generation | No |

<Note>
  Mistral handles reasoning natively through thinking chunks. While you don't configure reasoning effort explicitly, models that support it produce reasoning content automatically. Reasoning parts appear in `result.parts` and as `reasoning-start`, `reasoning-delta`, and `reasoning-end` stream events.
</Note>

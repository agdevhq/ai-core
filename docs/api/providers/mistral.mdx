...
<Note>
  Mistral models that support reasoning return thinking content in their responses. The `reasoning` config is accepted and reasoning parts are mapped to Mistral's native thinking chunks.
</Note>
...
const result = await generate({
  model: mistral.chatModel('mistral-large-2512'),
  messages: [
    { role: 'user', content: 'Solve this step by step: what is 127 * 43?' },
  ],
  reasoning: {
    effort: 'high',
  },

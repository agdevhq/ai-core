...
When you enable reasoning on Anthropic models, certain parameter restrictions apply:

- **temperature** must not be set
- **topP** must be between `0.95` and `1` (or omitted)
- **topK** (Anthropic provider option) must not be set
- **toolChoice** must be `'auto'` or `'none'` only

Violating these constraints throws a `ProviderError` at request time.

<Warning>
  When using both reasoning and tools, the `interleaved-thinking-2025-05-14` beta is automatically added to the request. You do not need to include it manually in the `betas` provider option.
</Warning>

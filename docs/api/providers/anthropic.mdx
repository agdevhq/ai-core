...
<Warning>
  ...
  - `temperature` must be `undefined` (do not set it)
  - `topP` must be between `0.95` and `1.0`, or `undefined`
  - `toolChoice` must be `'auto'` or `'none'`
  - The provider option `topK` must be `undefined`

  Violating any of these throws a `ProviderError`.
</Warning>

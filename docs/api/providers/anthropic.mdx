...
<AccordionGroup>
  ...
  <Accordion title="Manual Thinking (Claude 4.5 and earlier)" icon="brain">
...
When reasoning is enabled, Anthropic imposes the following restrictions:

- `temperature` must not be set
- `topP` must not be set (or must be between 0.95 and 1)
- `toolChoice` must be `'auto'` or `'none'` only
- `topK` (provider option) must not be set

...
<Note>
  `.result` resolves when the stream completes, regardless of whether you iterated the events. Streams are replayable -- iterating after events have arrived replays the buffered history.
</Note>

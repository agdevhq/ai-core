...
<Note>
  Streams start the HTTP request immediately when created -- you do not need to iterate to begin fetching. `.result` resolves when the stream completes, regardless of whether you consumed the events. Streams are replayable -- iterating after events have arrived replays the buffered history.
</Note>

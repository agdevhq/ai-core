...
<Note>
  `temperature`, `maxTokens`, and `topP` are top-level options. `stopSequences`, `frequencyPenalty`, and `presencePenalty` are provider-specific and passed via `providerOptions`. See [Configuration](/concepts/configuration) for details.
</Note>

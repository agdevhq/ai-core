...
<Update label="0.7.0" description="Require Zod 4, native JSON Schema conversion" tags={["breaking"]}>
  <Warning>This release contains breaking changes across all packages.</Warning>

  ### All packages

  - Require Zod 4 (`^4.0.0`) and replace `zod-to-json-schema` with Zod 4's native `z.toJSONSchema()`. The `zod-to-json-schema` library produced empty JSON schemas for Zod 4 schemas, breaking tool parameter conversion. A new `zodSchemaToJsonSchema` utility is exported from `@core-ai/core-ai` for consumers that need direct Zod-to-JSON-Schema conversion.
</Update>
...
<Update label="0.2.1" description="Zod compatibility" tags={["fix"]}>
  ### All packages

  - Add initial Zod 4 compatibility. Updates published Zod ranges and raises the minimum `zod-to-json-schema` version to one that supports Zod 4. (Zod 4 is now required as of v0.7.0.)
</Update>

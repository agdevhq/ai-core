...
<CardGroup cols={2}>
  ...
  >
    Streaming output with `stream()` and `ChatStream`. Shows how to handle real-time text generation with event-based streaming.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card
    title="Stream Abort"
    icon="circle-stop"
    href="https://github.com/agdevhq/core-ai/blob/main/examples/13-stream-abort.ts"
  >
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="o"
    href="https://github.com/agdevhq/core-ai/blob/main/examples/15-omnifact-provider.ts"
  >
    Using the Omnifact API Gateway with the same `generate()` API. Connect with your organization API key and any enabled model.
  </Card>
</CardGroup>
...
- **13-stream-abort.ts** - Abort a stream with `AbortSignal`
...
- **15-omnifact-provider.ts** - Omnifact API Gateway models

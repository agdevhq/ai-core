...
<Warning>
  Not all providers support all content types. For example, Anthropic only supports `application/pdf` file parts -- other MIME types throw an error. Check provider documentation for specific limitations.
</Warning>
...
The `providerMetadata` is provider-namespaced. The top-level key is the provider identifier (e.g. `'anthropic'`).

<Note>
  When reasoning parts from one provider are sent to a different provider, they are automatically downgraded to plain text wrapped in `<thinking>...</thinking>` tags. This ensures cross-provider compatibility in multi-turn conversations.
</Note>
...
Extract typed provider-specific metadata from a `providerMetadata` record:

```typescript
import { getProviderMetadata } from '@core-ai/core-ai';
import type { AnthropicReasoningMetadata } from '@core-ai/anthropic';

const result = await generate({ model, messages, reasoning: { effort: 'high' } });

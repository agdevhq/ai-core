...
The `providerMetadata` object is provider-namespaced. The top-level key is the provider identifier, such as `'anthropic'`, `'google'`, or `'openai'`.

<Note>
  Use [`getProviderMetadata()`](/api/core/utilities) when you want typed access to provider-specific metadata.
</Note>

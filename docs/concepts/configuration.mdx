...
- [OpenAI](/api/providers/openai) -- Responses API (`createOpenAI`): `store`, `serviceTier`, `include`, `parallelToolCalls`, `user`. Chat Completions API (`createOpenAICompat`): adds `stopSequences`, `frequencyPenalty`, `presencePenalty`, `seed`
- [Anthropic](/api/providers/anthropic) -- `topK`, `stopSequences`, `betas`, `outputConfig`
- [Google GenAI](/api/providers/google-genai) -- `stopSequences`, `frequencyPenalty`, `presencePenalty`, `seed`, `topK`
- [Mistral](/api/providers/mistral) -- `stopSequences`, `frequencyPenalty`, `presencePenalty`, `randomSeed`, `safePrompt`
...
<Note>
  For OpenAI, `stopSequences` is only available with `createOpenAICompat` (Chat Completions API). The default `createOpenAI` (Responses API) does not support it.
</Note>
...
```typescript
import { LLMError } from '@core-ai/core-ai';

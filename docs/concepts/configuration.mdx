...
- [OpenAI](/api/providers/openai) -- `stopSequences`, `frequencyPenalty`, `presencePenalty`, `seed`, `user`, and more
...
const result = await generate({
  ...
  temperature: 1.3,
  providerOptions: {
    openai: { frequencyPenalty: 0.7, presencePenalty: 0.4 },
  },
});
```

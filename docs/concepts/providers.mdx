...
const omnifact = createOmnifact({
  ...
  // baseURL: 'http://localhost:3001/v1/gateway', // local dev
});
```
...
```typescript
// List enabled models for your organization (OpenAI SDK example).
const models = await fetch('https://connect.omnifact.ai/v1/gateway/models', {
  headers: { Authorization: `Bearer ${process.env.OMNIFACT_API_KEY}` },
}).then((res) => res.json());

// Pass the returned id verbatim — EU-hosted models use the eu/ prefix.

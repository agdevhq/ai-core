...
```typescript
class StreamAbortedError extends LLMError {
  constructor(message?: string, cause?: unknown);
}
```
...
<Note>
  `StreamAbortedError` applies to an in-flight stream. Validation errors such as empty `messages` still throw `LLMError`.
</Note>
